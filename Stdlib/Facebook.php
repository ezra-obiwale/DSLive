...
class Facebook {
     ...
     * @return string
     */
    public static function init($appId, $locale = 'en_GB') {
        if (self::$initialized)
            return null;
        ob_start();
        ?>
     ...
     * @param string $website The address of the page to share
     ...
     * @return string
     */
    public static function share($website, $type = 'button_count', $class = null, $style = null) {
        if ($ret = self::notInitialized())
            return $ret;
        ob_start();
        ?>
        <div class="fb-share-button <?= $class ?>" style="<?= $style ?>" data-href="<?= $website ?>" data-type="<?= $type ?>"></div>
        <?php
        return ob_get_clean();
    }
     ...
     * @param string $website The address of the page to like
     * @param array $options Keys include:<br />
     *      boolean <b>showFaces</b>[false]: Indicates whether to show the faces of those 
     * like the page<br />
     ...
     * @return string
     */
    public static function like($website, array $options = array()) {
        if ($ret = self::notInitialized())
            return $ret;
        ...
        ob_start();
        ?>
        <div class="fb-like <?= @$options['class'] ?>" style="<?= @$options['style'] ?>" data-href="<?= $website . $_SERVER['REQUEST_URI'] ?>" data-width="<?= $options['width'] ?>" data-layout="<?= $options['layout'] ?>" data-action="<?= $options['action'] ?>" data-show-faces="<?= $options['showFaces'] ?>" data-share="<?= $options['share'] ?>"></div>
        <?php
        return ob_get_clean();
    }
     ...
     * @return string
     */
    public static function comments($website, $class = null, $numPosts = 10, $colorScheme = 'light') {
        if ($ret = self::notInitialized())
            return $ret;
        ob_start();
        ?>
        <div class="fb-comments <?= $class ?>" data-href="<?= $website . $_SERVER['REQUEST_URI'] ?>" data-numposts="<?= $numPosts ?>" data-colorscheme="<?= $colorScheme ?>"></div>
        <script>
            $(function() {
