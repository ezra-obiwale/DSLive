...
//                ->join('page')
		...
		<ul class="nav cms-nav">
			...
			foreach ($categories as $category) {
				...
				foreach ($pages as $page) {
					if ($id === $page->id) $active = true;
					?>
					<li  <?= ($id === $page->id) ? 'class="active"' : '' ?>><a data-id="<?= $page->id ?>" class="auto" tabindex="-1" href="<?= $renderer->url('cms', 'page', 'view', array(
						...
						if ($subCategories->count()) {
							foreach ($subCategories as $sub) {
								...
								foreach ($pages as $page) {
									if ($id === $page->id) $active = true;
									?>
									<li <?= ($id === $page->id) ? 'class="active"' : '' ?>><a tabindex="-1" class="auto" href="<?= $renderer->url('cms', 'page', 'view', array(
																		  $sub->link, $page->link))
									?>"><?= $page->title ?></a></li>
										<?php
									}
	...
	public static function cleanContent($content) {
		$sep = '_:DS:_';
		if ($cont = self::insertSlides($content, $sep)) $content = $cont;
	...
	public static function insertSlides($content, $sep = '_:DS:_', $attrs = array()) {
		...
		foreach ($slides as $slide) {
			...
			if ($slide->getHeight()) {
				if (isset($attrs['style'])) $attrs['style'] .= ';height:' . $slide->getHeight();
				else $attrs['style'] = 'height:' . $slide->getHeight();
			}
			$attrs['name'] = false;
	...
	public static function insertForms($content, $sep = '_:DS:_') {
		foreach (self::getForms($sep, $content) as $formAttrs) {
			if (empty($formAttrs)) continue;

			if ($formModel = \Session::fetch('form')) {
				\Session::remove('form');
			}
			else $formModel = new $formAttrs[1];
