...
class GuestController extends AController {
    ...
    private function register() {
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid() && $this->service->register($this->view,
                            array(
                        'module' => $this->module(),
    ...
    public function resendConfirmationAction($id) {
        ...
        if ($model->getActive()) {
            $this->flash()->setMessage('This account has already been confirmed')
                    ->addMessage('Please ' . str_replace('-', ' ',
                                    \Util::camelToHyphen($this->loginAction)));
        }
    ...
    public function loginAction($module = null, $controller = null,
        ...
        if (!$this->userIdentity()->isGuest()) {
            if ($module !== null) {
                ...
                $this->redirect($module, $controller,
                        $action ? $action : $this->userIdentity()->getUser()->getRole(),
                        $params);
            }
            else
                    $this->redirect('in', 'dashboard',
                        $this->userIdentity()->getUser()->getRole());
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid() && $model = $this->service->login($form->getModel())) {
                ...
                $controller = ($redirect['controller']) ? $redirect['controller']
                            : $controller;
                $action = ($redirect['action']) ? $redirect['action'] : $action;
                $params = ($redirect['params']) ? $redirect['params'] : explode(':',
                                $params);

                $this->redirect($module ? $module : $this->getModule(),
                        $controller ?
                                $controller : 'dashboard',
                        $action ? $action : $model->getRole(), $params);
            }
    ...
    public function resetPasswordAction($id = null, $resetId = null) {
        ...
        if ($this->request->isPost()) {
            $form->setData($this->request->getPost());
            if ($form->isValid() && $this->service->resetPassword($form->getModel(),
                            $id, $resetId)) {
    ...
    public function logoutAction($module = null, $controller = null,
            $action = null, $params = null) {
