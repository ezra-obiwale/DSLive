...
abstract class SuperService extends AService {
	...
	public function findOneWhere($criteria, $exception = true, $returnType = Table::RETURN_MODEL) {
		...
		if (!$model) {
			if ($exception) throw new Exception('Required page was not found');
			else $this->addErrors('Required object was not found');
		} else {
			$this->model = $model;
		}
		return $model;
	...
	public function save(IModel $model, Object $files = null, $flush = true) {
		...
		$upload = true;
		if ($this->checkFileIsNotEmpty($files->toArray(true)) && method_exists($model, 'uploadFiles') && !$upload = $model->uploadFiles($files)) {
			$this->addErrors('File upload failed');
			$this->addErrors($model->getErrors());
			return false;
		}
	...
	public function import(Object $data) {
		if (empty($data->file->name) || $data->file->error) return false;
