...
abstract class GuestService extends AService {
	...
	public function getLoginForm($passwordResetLink = null) {
		if (!$this->loginForm) $this->loginForm = new LoginForm($passwordResetLink);
	...
	public function getResetPasswordForm() {
		if (!$this->resetPasswordForm) $this->resetPasswordForm = new ResetPasswordForm();
	 ...
	 * @return boolean
	 */
	public function register(View $view, $controllerPath, Form $form, $setup = false, $sendMail = true,
						  $flush = true) {
		$model = $form->getModel();
		$model->hashPassword();
		...
		return $this->registerUser($model, $view, $controllerPath, $setup, $sendMail, $flush);
	}

	private function registerUser(User $user, View $view, $controllerPath, $setup, $sendMail, $flush) {
		if ($this->repository->insert($user)->execute()) {
			if ($sendMail && engineGet('server') !== 'development' && !$this->sendEmail($user)) {
				$this->addErrors('Confirmation email not sent. <a href="' .
								$view->url($controllerPath['module'], $controllerPath['controller'], 'resend-confirmation', array(
									$user->getId()))) .
	...
	public function mediaSignup(\Object $data, View $view, $controllerPath, $sendMail = true,
							 $flush = true) {
		...
		if ($model) {
			...
			return $this->registerUser($this->model, $view, $controllerPath, false, $sendMail, $flush);
		}
	}
	...
	public function sendEmail(User $user, $notifyType = self::NOTIFY_REG) {
		...
		$view = new View();
		$links = array(
			'{confirmationLink}' => engineGet('config', 'app', 'domain') . $view->url($this->getModule(), $this->getClassName(), 'confirm-registration', array(
				$user->getId(), $user->getEmail())),
			'{loginLink}' => engineGet('config', 'app', 'domain') . $view->url($this->getModule(), $this->getClassName(), 'login'),
			'{passwordResetLink}' => engineGet('config', 'app', 'domain') . $view->url($this->getModule(), $this->getClassName(), 'reset-password', array(
				$user->getId(), $user->getReset())),
		);
		...
		if ($webMasterEmail) $email->sendFrom($webMasterEmail, $notification->getGetReplies());
		else
	...
	final public function addErrors($error) {
		...
		else if (is_array($error)) $this->errors = array_merge($this->errors, $error);
