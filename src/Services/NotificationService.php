...
class NotificationService extends SuperService {
	...
	public function createRegistrationMessage() {
		...
		Table::addRowData('You have successfully registered with ' . engineGet('config', 'app', 'name') .
				'. Please click on the link below to confirm your registration and activate your account.', array(
			'colspan' => 2));
		Table::newRow();
		Table::addRowData('<a href="{confirmationLink}">{confirmationLink}</a>', array('colspan' => 2));
		Table::newRow();
		Table::addRowData('You may copy and and paste the link into your browser address if it is not clickable.', array(
			'colspan' => 2));
		Table::newRow();
	...
	public function createConfirmationMessage() {
		...
		Table::addRowData('You account with ' . engineGet('config', 'app', 'name') .
				' has been successfully confirmed and activated. <a href="{loginLink}">Click here to login now</a>', array(
			'colspan' => 2));
		Table::newRow();
		Table::addRowData('You may copy and and paste the link below into your browser address if the one above is not clickable.', array(
			'colspan' => 2));
		Table::newRow();
	...
	public function createPasswordResetMessage() {
		...
		Table::addRowData('&nbsp;', array('colspan' => 2));
		Table::newRow();
		Table::addRowData('PS: If you have not requested a password reset, please ignore this message.', array(
			'colspan' => 2));
		Table::newRow();
		Table::addRowData('Thank you', array('colspan' => 2));
		Table::newRow();
	...
	public function createPasswordResetSuccessMessage() {
		...
		Table::addRowData('<ul>'
				...
				. '</ul>', array('colspan' => 2));
		Table::newRow();
		Table::addRowData('Below is the said password reset link. Click on it to reset your password', array(
			'colspan' => 2));
		Table::newRow();
		Table::addRowData('<a href="{passwordResetLink}">{passwordResetLink}</a>', array('colspan' => 2));
		Table::newRow();
